...
/* ========================================================================
 * Shows progress in current file
 */
void
display_progress(void)
{
  static char printfbuf[200] = {'\0'};
  static off_t fstart;
  static off_t bytes_to_process;
  static int buflen;
  static char* last_ifname = 0;
  
  if(last_ifname == 0) last_ifname = calloc(1, 1);

  if(strcmp(last_ifname, ifname) != 0) {
    /* first time displaying info about this file */
    int ifname_len = strlen(ifname);
    last_ifname = realloc(last_ifname, ifname_len + 1);
    last_ifname[ifname_len] = '\0';
    memcpy(last_ifname, ifname, ifname_len);

    fstart = lseek(ifd, 0, SEEK_CUR);
    bytes_to_process = lseek(ifd, 0, SEEK_END) - fstart;
    lseek(ifd, fstart, SEEK_SET);
  }
  else {
    /* move cursor left to overwrite info from last time */
    memset(printfbuf, '\b', buflen);
    fprintf(stderr, printfbuf);
  }

  off_t bytes_done = lseek(ifd, 0, SEEK_CUR) - fstart;

  if(bytes_to_process < 2000) {
    sprintf(printfbuf, "%.1f%%%n",
      (float)bytes_done/bytes_to_process*100, &buflen);
  }
  else if(bytes_to_process < 2000000) {
    sprintf(printfbuf, "%.1fKB/%.1fKB (%.1f%%)%n",
      (float)bytes_done/1000, (float)bytes_to_process/1000,
      (float)bytes_done/bytes_to_process*100, &buflen);
  }
  else if(bytes_to_process < 2000000000) {
    sprintf(printfbuf, "%.1fMB/%.1fMB (%.1f%%)%n",
      (float)bytes_done/1000000, (float)bytes_to_process/1000000,
      (float)bytes_done/bytes_to_process*100, &buflen);
  }
  else {
    sprintf(printfbuf, "%.1fGB/%.1fGB (%.1f%%)%n",
      (float)bytes_done/1000000000, (float)bytes_to_process/1000000000,
      (float)bytes_done/bytes_to_process*100, &buflen);
  }
